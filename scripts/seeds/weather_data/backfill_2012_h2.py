#!/usr/bin/env python3
"""
Backfill missing weather data for 2012 second half (July-December) only.
"""
...
async def get_existing_dates() -> Set:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(text(f"""
            ...
            WHERE hour >= '{YEAR}-{START_MONTH:02d}-01' AND hour < '{YEAR + 1}-01-01'
            ORDER BY d
        """))
...
async def fetch_era5_for_day(date: datetime, windfarms):
    ...
    grib_dir = Path(__file__).parent.parent.parent.parent / 'grib_files' / f'daily_{YEAR}_h2'
...
async def main():
    logger.info("="*60)
    logger.info(f"BACKFILL {YEAR} H2 (Jul-Dec) WEATHER DATA")
    logger.info("="*60)

    existing_dates = await get_existing_dates()
    logger.info(f"Found {len(existing_dates)} days already with data in H2")

    all_dates = []
    current = datetime(YEAR, START_MONTH, 1, tzinfo=timezone.utc)
    ...
    if total_days == 0:
        logger.info(f"{YEAR} H2 is complete!")
        return
