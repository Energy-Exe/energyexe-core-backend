...
async def list_users():
    ...
    async with async_session_factory() as db:
        ...
        for user in users:
            print(f"{user.id:<4} {user.email:<30} {user.username:<20} "
...
async def main():
    ...
    parser.add_argument('--create', action='store_true', help='Create new superuser instead of promoting existing')
    ...
    if args.create:
        ...
        success = await promote_user_to_superuser(
            email=args.email,
            username=args.username
        )
