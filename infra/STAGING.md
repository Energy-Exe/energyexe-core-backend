...
The frontends are different origins from the API (`staging-dashboard` / `staging-app` →
`staging-api`), so the browser enforces CORS. `BACKEND_CORS_ORIGINS` is set (via the `cors_origins`
tfvar → the ECS task env) to **exactly the two staging origins**, as a JSON-array string:
`["https://staging-dashboard.energyexe.com","https://staging-app.energyexe.com"]`.

**It must be an explicit allowlist, never `*`.** The app authenticates with a bearer token in the
`Authorization` header (no cookie) and the client makes credentialed requests, so the API responds
with `Access-Control-Allow-Credentials: true`. A wildcard `Access-Control-Allow-Origin: *` combined
with credentials is rejected by browsers — the API must echo the *specific* calling origin (`Vary:
Origin`). Adding a new frontend origin = add it to `cors_origins` + `terraform apply`. (See §6 for
the wildcard bug this caused and the one-line fix.)
...
- **CORS wildcard + credentials is broken for token auth.** The API had `allow_origins=["*"]` with
  `allow_credentials=True`. With no cookie on the request (bearer-token auth), Starlette emits
  `Access-Control-Allow-Origin: *` on the *actual* (non-preflight) response, which browsers reject for
  credentialed requests — so the frontends got "CORS error" on every call even though the **preflight
...
**Set the backend CORS origin:** set `cors_origins` (JSON-array string — an explicit allowlist, never
`*`; see §6) + optional `*_portal_url` in `terraform.tfvars`, then `terraform apply` (replaces the
task def → brief staging redeploy).
...
- **Staging frontends LIVE on AWS:** `https://staging-dashboard.energyexe.com` (admin-ui) +
  ...
  back correctly, and the bundles call `staging-api.energyexe.com`. Deployed by each repo's
  `deploy-staging.yml` on push to `staging`.
- **CORS fixed** so the AWS frontends can actually call the staging API: `BACKEND_CORS_ORIGINS` is the
  explicit two-origin allowlist and the API echoes the specific calling origin (backend PR #135 — the
  earlier `*` + credentials combo was rejected by browsers). Verified: both origins allowed, unknown
  origins refused.
