    ...
    async def get_windfarm_statistics(
        ...
        for row in rows:
            ...
            stats.append({
                ...
                'capacity_mw': float(row.nameplate_capacity_mw) if row.nameplate_capacity_mw else None,
                'total_generation_mwh': float(row.total_generation) if row.total_generation else 0,
                'avg_generation_mwh': float(row.avg_generation) if row.avg_generation else 0,
