...
class MarketBalanceAreaService:
    ...
    async def get_market_balance_area(
        db: AsyncSession, market_balance_area_id: int
    ...
    async def search_market_balance_areas(
        ...
        result = await db.execute(
            select(MarketBalanceArea)
            .options(selectinload(MarketBalanceArea.country))
            .where(and_(MarketBalanceArea.name.ilike(search_pattern)))
            .offset(skip)
            .limit(limit)
            .order_by(MarketBalanceArea.created_at.desc())
        )
    ...
    async def create_market_balance_area(
        db: AsyncSession, market_balance_area: MarketBalanceAreaCreate
    ) -> MarketBalanceArea:
    ...
    async def update_market_balance_area(
        ...
        result = await db.execute(
            select(MarketBalanceArea).where(MarketBalanceArea.id == market_balance_area_id)
        )
