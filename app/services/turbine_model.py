...
class TurbineModelService:
    ...
    async def search_turbine_models(
        ...
        result = await db.execute(
            ...
            .where(
                and_(
                    TurbineModel.model.ilike(search_pattern)
                )
            )
            .offset(skip)
            .limit(limit)
    ...
    @staticmethod
    async def create_turbine_model(db: AsyncSession, turbine_model: TurbineModelCreate) -> TurbineModel:
    ...
    @staticmethod
    async def delete_turbine_model(db: AsyncSession, turbine_model_id: int) -> Optional[TurbineModel]:
        result = await db.execute(select(TurbineModel).where(TurbineModel.id == turbine_model_id))
        db_turbine_model = result.scalar_one_or_none()
