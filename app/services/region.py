...
class RegionService:
    ...
    async def get_regions(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Region]:
        result = await db.execute(
            select(Region).offset(skip).limit(limit).order_by(Region.created_at.desc())
        )
    ...
    async def search_regions(
        ...
        result = await db.execute(
            select(Region)
            .where(and_(Region.name.ilike(search_pattern)))
            .offset(skip)
            .limit(limit)
    ...
    async def update_region(
        db: AsyncSession, region_id: int, region_update: RegionUpdate
    ) -> Optional[Region]:
        result = await db.execute(select(Region).where(Region.id == region_id))
