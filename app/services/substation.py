...
class SubstationService:
    ...
    async def get_substations(
        ...
        result = await db.execute(
            select(Substation).offset(skip).limit(limit).order_by(Substation.created_at.desc())
        )
    ...
    async def search_substations(
        ...
        result = await db.execute(
            select(Substation)
            .where(and_(Substation.name.ilike(search_pattern)))
            .offset(skip)
            .limit(limit)
    ...
    async def update_substation(
        db: AsyncSession, substation_id: int, substation_update: SubstationUpdate
    ) -> Optional[Substation]:
        result = await db.execute(select(Substation).where(Substation.id == substation_id))
