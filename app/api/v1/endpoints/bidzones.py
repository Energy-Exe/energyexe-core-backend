...
async def get_bidzone(bidzone_id: int, db: AsyncSession = Depends(get_db)):
...
async def get_bidzone_by_code(code: str, db: AsyncSession = Depends(get_db)):
...
async def create_bidzone(bidzone: BidzoneCreate, db: AsyncSession = Depends(get_db)):
    ...
    if existing_bidzone:
        raise HTTPException(status_code=400, detail="Bidzone with this code already exists")
...
async def update_bidzone(
    bidzone_id: int, bidzone_update: BidzoneUpdate, db: AsyncSession = Depends(get_db)
):
    """Update a bidzone"""
...
async def delete_bidzone(bidzone_id: int, db: AsyncSession = Depends(get_db)):
    """Delete a bidzone"""
