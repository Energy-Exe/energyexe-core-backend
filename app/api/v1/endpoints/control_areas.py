...
async def get_control_area(control_area_id: int, db: AsyncSession = Depends(get_db)):
...
async def get_control_area_by_code(code: str, db: AsyncSession = Depends(get_db)):
...
async def create_control_area(control_area: ControlAreaCreate, db: AsyncSession = Depends(get_db)):
    ...
    if existing_control_area:
        raise HTTPException(status_code=400, detail="Control area with this code already exists")
...
async def update_control_area(
    control_area_id: int, control_area_update: ControlAreaUpdate, db: AsyncSession = Depends(get_db)
):
    ...
    if control_area_update.code:
        existing_control_area = await ControlAreaService.get_control_area_by_code(
            db, control_area_update.code
        )
        if existing_control_area and existing_control_area.id != control_area_id:
            raise HTTPException(
                status_code=400, detail="Control area with this code already exists"
            )

    updated_control_area = await ControlAreaService.update_control_area(
        db, control_area_id, control_area_update
    )
...
async def delete_control_area(control_area_id: int, db: AsyncSession = Depends(get_db)):
