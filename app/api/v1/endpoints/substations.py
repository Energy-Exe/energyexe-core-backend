...
async def get_substation_by_code(
    code: str,
    db: AsyncSession = Depends(get_db)
):
...
async def create_substation(
    ...
    if existing_substation:
        raise HTTPException(
            status_code=400,
            detail="Substation with this code already exists"
        )
...
async def update_substation(
    ...
    if substation_update.code:
        existing_substation = await SubstationService.get_substation_by_code(db, substation_update.code)
    ...
    updated_substation = await SubstationService.update_substation(db, substation_id, substation_update)
