...
async def get_turbine_model_by_model(
    model: str,
    db: AsyncSession = Depends(get_db)
):
...
async def create_turbine_model(
    turbine_model: TurbineModelCreate,
    db: AsyncSession = Depends(get_db)
):
    """Create a new turbine model"""
    # Check if turbine model with same model name already exists
    existing_turbine_model = await TurbineModelService.get_turbine_model_by_model(db, turbine_model.model)
    if existing_turbine_model:
        raise HTTPException(
            status_code=400,
            detail="Turbine model with this model name already exists"
        )
...
async def update_turbine_model(
    ...
    if turbine_model_update.model:
        existing_turbine_model = await TurbineModelService.get_turbine_model_by_model(db, turbine_model_update.model)
    ...
    updated_turbine_model = await TurbineModelService.update_turbine_model(db, turbine_model_id, turbine_model_update)
