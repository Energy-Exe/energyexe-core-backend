...
async def get_cable_by_code(code: str, db: AsyncSession = Depends(get_db)):
...
async def create_cable(cable: CableCreate, db: AsyncSession = Depends(get_db)):
    """Create a new cable"""
    # Check if cable with same code already exists
    existing_cable = await CableService.get_cable_by_code(db, cable.code)
    if existing_cable:
        raise HTTPException(status_code=400, detail="Cable with this code already exists")
...
async def update_cable(
    cable_id: int, cable_update: CableUpdate, db: AsyncSession = Depends(get_db)
):
    """Update a cable"""
