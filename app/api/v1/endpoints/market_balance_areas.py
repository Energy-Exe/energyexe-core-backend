...
async def search_market_balance_areas(
    ...
    return await MarketBalanceAreaService.search_market_balance_areas(db, query=q, skip=skip, limit=limit)


@router.get("/{market_balance_area_id}", response_model=MarketBalanceArea)
async def get_market_balance_area(
    market_balance_area_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get a specific market balance area by ID"""
    market_balance_area = await MarketBalanceAreaService.get_market_balance_area(db, market_balance_area_id)
...
async def get_market_balance_area_by_code(
    code: str,
    db: AsyncSession = Depends(get_db)
):
...
async def create_market_balance_area(
    market_balance_area: MarketBalanceAreaCreate,
    db: AsyncSession = Depends(get_db)
):
    """Create a new market balance area"""
    # Check if market balance area with same code already exists
    existing_market_balance_area = await MarketBalanceAreaService.get_market_balance_area_by_code(db, market_balance_area.code)
    if existing_market_balance_area:
        raise HTTPException(
            status_code=400,
            detail="Market balance area with this code already exists"
        )
...
async def update_market_balance_area(
    ...
    if market_balance_area_update.code:
        existing_market_balance_area = await MarketBalanceAreaService.get_market_balance_area_by_code(db, market_balance_area_update.code)
        if existing_market_balance_area and existing_market_balance_area.id != market_balance_area_id:
            raise HTTPException(
                status_code=400,
                detail="Market balance area with this code already exists"
            )
    
    updated_market_balance_area = await MarketBalanceAreaService.update_market_balance_area(db, market_balance_area_id, market_balance_area_update)
...
async def delete_market_balance_area(
    ...
    deleted_market_balance_area = await MarketBalanceAreaService.delete_market_balance_area(db, market_balance_area_id)
