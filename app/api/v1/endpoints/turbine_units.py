...
async def get_turbine_unit_by_code(
    code: str,
    db: AsyncSession = Depends(get_db)
):
...
async def create_turbine_unit(
    turbine_unit: TurbineUnitCreate,
    db: AsyncSession = Depends(get_db)
):
    ...
    if existing_turbine_unit:
        raise HTTPException(
            status_code=400,
            detail="TurbineUnit with this code already exists"
        )
...
async def update_turbine_unit(
    ...
    if turbine_unit_update.code:
        existing_turbine_unit = await TurbineUnitService.get_turbine_unit_by_code(db, turbine_unit_update.code)
    ...
    updated_turbine_unit = await TurbineUnitService.update_turbine_unit(db, turbine_unit_id, turbine_unit_update)
